 ...
 */
/**
 * ⚠️ TROVATO DAL VIVO IL 18 AGOSTO 2026: `calendar/presence_status` guarda
 * solo l'istante presente (le timbrature vere) — per un giorno futuro
 * («chi è in ferie lunedì?») torna sempre vuoto, perché lunedì nessuno ha
 * ancora timbrato niente. Le ferie/permessi/malattie sono una cosa
 * PROGRAMMATA, non una timbratura: la porta giusta è `calendar/company`,
 * che per ogni contratto dice il tipo di giornata programmata
 * (`COMMON.HOLIDAYS`, `COMMON.PERMITS`, `COMMON.DISEASES`, o "work") — e
 * funziona sia per oggi che per il futuro, con la stessa chiamata.
 */
const TIPO_LEGGIBILE: Record<string, string> = {
  "COMMON.HOLIDAYS": "in ferie",
  "COMMON.PERMITS": "in permesso",
  "COMMON.DISEASES": "in malattia",
};

interface VoceCalendario {
  contract_id: string;
  schedule?: { type_name: string | null }[];
  flexible_schedule?: { type_name: string | null }[];
}
...
export async function presenzeGiorno(azienda: string, giorno?: string): Promise<string> {
  ...
  const giornoValido = giorno && /^\d{4}-\d{2}-\d{2}$/.test(giorno) ? giorno : oggiIso;
  const descrizioneGiorno =
    giornoValido === oggiIso
      ? "oggi"
  ...
  try {
    const [rCalendario, rContratti] = await Promise.all([
      fetch(
        `${FLUIDA}/calendar/company/${companyId}?start_date=${giornoValido}&end_date=${giornoValido}`,
        { headers: intestazione, signal: AbortSignal.timeout(15_000) }
      ),
    ...
    if (!rCalendario.ok) {
      const errore = `Fluida ha risposto ${rCalendario.status} alla richiesta del calendario.`;
      await getPool().query("select public.az_fluida_errore($1,$2)", [azienda, errore]);
      return errore;
    }

    const calendario = (await rCalendario.json()) as { data?: VoceCalendario[] };
    ...
    const contratti = rContratti.ok
      ? ((await rContratti.json()) as {
          data?: { id: string; firstname: string | null; lastname: string | null; job_title: string | null; active: boolean }[];
        })
      : { data: [] };
    const nomi = new Map(
      (contratti.data ?? [])
        .filter((c) => c.active)
        .map((c) => [c.id, [c.firstname, c.lastname].filter(Boolean).join(" ") || c.job_title || c.id])
    );

    const righe = (calendario.data ?? [])
      .filter((v) => nomi.has(v.contract_id))
      .map((v) => {
        const tipi = [...(v.schedule ?? []), ...(v.flexible_schedule ?? [])]
          .map((s) => s.type_name)
          .filter((t): t is string => t != null && t in TIPO_LEGGIBILE);
        return { chi: nomi.get(v.contract_id)!, stato: tipi.length ? TIPO_LEGGIBILE[tipi[0]] : "al lavoro" };
      });
    if (righe.length === 0) return `Fluida non ha restituito nessun dato per ${descrizioneGiorno}.`;

    righe.sort((a, b) => a.chi.localeCompare(b.chi, "it"));
    return (
      `Programma di ${descrizioneGiorno} da Fluida (riporta i nomi ESATTAMENTE come scritti qui sotto; ` +
      `chi non è elencato come "in ferie/permesso/malattia" è considerato al lavoro):\n` +
