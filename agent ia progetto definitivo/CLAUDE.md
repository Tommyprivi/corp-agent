...
Piattaforma SaaS che permette a chiunque di creare "lavoratori digitali IA" chattando.
Zero codice, zero sbattimento. Product Manager e proprietario del design: Tommaso. Il
codice lo scrive Claude.

## Leggi prima di iniziare, in quest'ordine

1. **[docs/PERCORSO.md](docs/PERCORSO.md) — L'ORDINE DI LAVORO VINCOLANTE.** Fissato da
   Tommaso il 1 Agosto 2026. Le 108 funzioni del documento, ordinate per dipendenza
   tecnica, divise in 10 fasi. **Si segue questo ordine, non se ne inventano altri.**
...
3. [docs/BACKLOG.md](docs/BACKLOG.md) — tutto ciò che SPEC.md descrive ma che è stato
   congelato per dopo il lancio. Se un'idea sembra mancare, cercala qui prima di
   riproporla: probabilmente è già stata vista e rimandata di proposito.
4. [docs/SPEC.md](docs/SPEC.md) — trascrizione grezza del documento di Tommaso. Contiene
   sia la bibbia sia il backlog, non filtrati. Non riscriverlo.

## La regola d'oro

**"Se una feature non aiuta a vendere entro Dicembre 2026, tagliala."** Prima di
aggiungere qualunque cosa non elencata nel Passo attivo di PIANO.md, fermarsi e chiedere,
non semplicemente costruirla perché è nel documento sorgente.
...
Il type-check deve essere pulito prima di chiudere una sessione.

⚠️ **Non usare `tsc --noEmit`**: `tsconfig.json` ha `"files": []` con references, quindi quel
comando non controlla niente e passa sempre. Serve la modalità build (`tsc -b`).

## Stack

**Davanti:** React 19 + Vite + TypeScript + Tailwind v4 (`@tailwindcss/postcss`). Nessuna
libreria di componenti: le icone sono SVG scritti a mano in `src/components/Icons.tsx`.

**Dietro** (deciso da Tommaso il 1 Agosto 2026, sostituisce Supabase):

| Pezzo | Scelta |
|---|---|
| Database | **Neon** — PostgreSQL puro in cloud, amministrato con pgAdmin, `pgvector` per il RAG |
| Server e chiavi | **funzioni su Vercel**, cartella `api/`, firma web standard `export default { fetch }` |
| Accessi | **Better Auth** con Google e Apple, utenti dentro Neon |
| Modelli | **OpenRouter** |
| Pagamenti | **Stripe** (Fase 4) |

⚠️ Vercel **non** supporta i "project references" del `tsconfig.json` per le funzioni:
nella cartella `api/` si usano solo import relativi, mai alias di percorso.
...
```
src/                     quello che si vede (browser)
  types.ts               tipi condivisi
  data/                  cataloghi: trades, agenti preimpostati, modelli, connettori, piani
  lib/recommend.ts       motore di raccomandazione a 3 livelli (agente / connettore / piano)
  components/
    MasterBuilder.tsx    l'unica cosa che si vede al primo ingresso
    WhatsAppConnect.tsx  connessione canale via QR, zero attrito
    views/               Home, ControlCenter (chat), Advanced

api/                     quello che gira sul server (Vercel) — qui vivono le chiavi
  _lib/db.ts             collegamento a Neon + withUser() per la sicurezza per riga
  _lib/auth.ts           Better Auth (Google, Apple)
  _lib/openrouter.ts     scelta automatica del modello, prezzi, avviso costi
  auth/[...all].ts       tutti gli indirizzi degli accessi
  chat.ts                la risposta in streaming + conteggio consumi + salvataggio
  models.ts              catalogo modelli per il frontend

db/migrations/           SQL versionato, da eseguire in pgAdmin
```

I file di `api/` che iniziano con `_` non diventano indirizzi pubblici: è la convenzione di
Vercel per il codice condiviso.

Tre voci di navigazione, fissate dalla bibbia: **Home**, **Chat**, **Impostazioni
Avanzate**. Non ce ne sono altre — se serve una quarta voce, è quasi certamente backlog.

## Il principio guida

**L'utente non deve vedere 100 funzioni.** Entra e parla con il Master Builder, che gli
...
Questo e il contenuto completo di `AgentFlow.docx`, copiato qui dentro su richiesta
esplicita di Tommaso perche faccia parte delle regole lette a ogni sessione.

**Come leggerlo:** contiene sia la bibbia del progetto (le prime righe, che vincono su
tutto) sia un brainstorming enorme di funzioni. Cosa e V1 e cosa e backlog e deciso in
[docs/PIANO.md](docs/PIANO.md) e [docs/BACKLOG.md](docs/BACKLOG.md). La regola d oro resta
il primo filtro: se non aiuta a vendere entro Dicembre 2026, va in backlog.
...
prova corpagent poi entri ti chiede di fare l accesso con google apple ecc. poi accetti il captcha e arrivi ad una nuova schermata con scritto come ci hai conosciuto con queste opzioni :
...
- Il "White-Label" Dinamico per Agenzie: Permette a un'agenzia di marketing o a un commercialista di rivendere CorpAgent ai propri clienti applicando il proprio logo, i propri colori e impostando prezzi personalizzati.
...
- Il Generatore di Widget Incorporabili (Embed Code): Con un click la piattaforma genera una riga di codice HTML che l'utente può incollare sul suo sito web esistente (WordPress, Shopify, Squarespace) per far comparire la chat di CorpAgent in stile bolla di assistenza in basso a destra.
...
La vera bellezza di CorpAgent è che gli agenti IA parlano qualsiasi lingua per natura.
...
Immagina un'azienda italiana che usa CorpAgent collegato a WhatsApp: un cliente straniero scrive in cinese.
...
- API Key per Sviluppatori: Permetti ai clienti di integrare gli agenti di CorpAgent direttamente all'interno dei loro software proprietari o di siti web esterni, aprendo a un modello di business B2B avanzato (White-label o API-as-a-Service).
- Webhooks in Uscita: La capacità di CorpAgent di "chiamare" un URL esterno ogni volta che un agente completa un task con successo, notificando software terzi in tempo reale.
...
L'utente entra nella dashboard, clicca su "Crea nuovo agente con l'IA" e si apre una chat con il Master Builder (l'agente principale di CorpAgent).
...
li utenti e le aziende non possono stare tutto il giorno incollati alla dashboard di CorpAgent ad aspettare che l'agente finisca un task o risponda a un cliente.
...
Per fare in modo che le aziende non disdicano l'abbonamento (basso Churn Rate), devono vedere chiaramente il valore economico e di tempo che CorpAgent sta portando nel loro business.
...
- Monetizzazione per i Creator: Gli utenti possono mettere in vendita i loro agenti o offrirli gratis, e tu trattieni una percentuale (commissione) su ogni transazione, trasformando CorpAgent in una piattaforma stile "App Store" per agenti IA.

Multi-Tenant White-Label (Rivendita per Agenzie)
- Cos'è: Permette ad agenzie di marketing, software house o consulenti di acquistare un piano speciale per "rimarchiare" CorpAgent con il proprio logo, i propri colori e il proprio dominio (es. clienti.agenzia.it).
...
- Cos'è: La possibilità di creare due varianti dello stesso agente con due prompt leggermente diversi (es. uno con un tono molto formale, uno più amichevole) e fare in modo che CorpAgent divida il traffico dei clienti al 50%.
...
Ecco come gestiamo questa funzione a 360 gradi in CorpAgent:
...
- Come funziona: Colleghi CorpAgent ai calendari aziendali (Google Calendar, Outlook). Quando un cliente scrive su WhatsApp o via email chiedendo un appuntamento, l'agente controlla gli orari liberi del team, negozia l'appuntamento con il cliente e crea l'evento in automatico inviando gli inviti.
...
Invece di costringere l'imprenditore a partire da un foglio bianco o a configurare l'agente da zero, CorpAgent offrirà dei pacchetti pre-configurati per ogni nicchia di mercato.
...
- Come funziona: Permetti a qualsiasi utente (o a web agency, commercialisti, consulenti) di avere un link di affiliazione unico. Se un consulente consiglia CorpAgent a un suo cliente e quello si abbona al piano Pro (49€) o Enterprise (230€), l'affiliato riceve una commissione ricorrente mensile a vita (es. il 20% o 30% ogni mese).
...
- Come funziona: Una pagina pubblica sul sito di CorpAgent dove un potenziale cliente inserisce il link del suo sito web o descrive la sua attività. Un'IA analizza i suoi processi aziendali (es. quanto ci mette a rispondere ai clienti, quanta burocrazia ha) e genera un Report di Inefficienza Gratuito di 3 pagine, mostrando esattamente dove sta perdendo soldi e quale agente di CorpAgent risolverebbe il problema.
...
- Come funziona: Un tool gratuito sul sito di CorpAgent dove un'azienda inserisce il proprio sito e quello di 2 concorrenti. Gli agenti analizzano i siti, le recensioni pubbliche e il posizionamento, inviando via email un report: "Ecco perché i tuoi concorrenti ti stanno superando e come il tuo agente Nexus può colmare il gap in 7 giorni".
...
- Come funziona: L'utente apre la web app mobile di CorpAgent, clicca sull'icona della fotocamera, scatta la foto a una pila di scontrini, a una bolletta o a una fattura cartacea. L'agente estrae automaticamente tutti i dati, li inserisce nel gestionale o nel foglio Excel e calcola i totali.
...
- Comodità totale per il titolare: L'imprenditore può gestire tutto o farsi aggiornare dall'agente mentre è in macchina, mentre cammina o mentre ha le mani occupate a lavorare, semplicemente usando la voce.
 Quindi ci sara corpagent su whastapp per mobile

Altre funzioni e idee di altissimo livello per potenziare l'ecosistema CorpAgent, pensate per rendere la piattaforma irresistibile per le aziende e massimizzare i profitti:
...
Integrare uno scanner intelligente direttamente nella web app di CorpAgent è una mossa fenomenale, soprattutto per liberi professionisti, artigiani, piccoli commercianti e ristoratori che hanno a che fare ogni giorno con montagne di carta.
...
- Perfetta per il mobile-first: Sfrutta al 100% lo strumento che l'imprenditore ha già in tasca (lo smartphone), rendendo CorpAgent uno strumento indispensabile nella vita lavorativa di tutti i giorni.
Altre funzioni potentissime, trasversali e applicabili a qualsiasi ambito aziendale (dalla ristorazione alla consulenza, dall'immobiliare all'artigianato), pensate per rendere CorpAgent un assistente universale e indispensabile:
...
Ecco la struttura completa e dettagliata delle impostazioni dell'organizzazione per CorpAgent. Quando un'azienda (o un'agenzia) crea la sua organizzazione sulla piattaforma, sblocca un pannello di controllo centralizzato per gestire persone, limiti, sicurezza e risorse.
...
Usare PostgreSQL come database relazionale (gestito tramite pgAdmin) e far girare tutto l'infrastruttura in cloud è la combinazione perfetta per una piattaforma come CorpAgent che deve scalare, gestire utenti multipli (fino a 200 postazioni per organizzazione) e garantire affidabilità.
...
Dato che CorpAgent gestisce dati aziendali critici, utenti, cronologie chat, file vettoriali e permessi aziendali, la sala macchine di tutto il software gira su PostgreSQL.
...
- Come si collega al software: Il backend di CorpAgent (scritto in Python/FastAPI) comunica costantemente con PostgreSQL tramite un ORM (come SQLAlchemy) per salvare ogni singola conversazione, aggiornare il wallet dei crediti e memorizzare i prompt personalizzati creati dagli utenti.
...
Altre 10 funzioni avanzate e professionali da integrare nell'app (non agenti, ma strumenti concreti di piattaforma) per spingere ancora più in alto il livello di CorpAgent:
...
- White-Label Completo (Rimozione Branding CorpAgent): Sostituzione totale del logo e dei riferimenti della piattaforma con quelli dell'agenzia o dell'azienda cliente.
...
Questi strumenti trasformano CorpAgent in un hub centrale di automazione e gestione dati, collegandolo a tutto l'ecosistema IT aziendale.
...
- API Rate Limiting Management: Impostazione di limiti sul numero di richieste che le integrazioni esterne possono fare alle API di CorpAgent.
...
- Pixel Facebook / Meta Conversions API: Invio di eventi di conversione (es. lead qualificato, acquisto) direttamente dai server di CorpAgent ai sistemi Meta.
...
- Gestione delle Transizioni di Pagina: Scelta tra diverse animazioni fluide (fade, slide, zoom) quando si naviga tra le diverse sezioni della dashboard CorpAgent.
...
- Rimozione Totale del Riferimento "Powered by CorpAgent": Opzione (sbloccabile nei piani superiori) per eliminare ogni traccia del brand CorpAgent dal codice sorgente del widget e dall'interfaccia.
...
- Dominio Email per Notifiche: Possibilità di inviare le notifiche automatiche ai clienti (es. follow-up, alert) utilizzando il dominio email dell'azienda (es. noreply@miosito.it invece di notifications@corpagent.com).
- White-Label delle App Mobile (iOS/Android): (Piano Enterprise) Possibilità di ottenere le app mobile di CorpAgent riconfigurate con il logo e il nome dell'azienda sugli store.
...
Hai perfettamente ragione. Con una mole di funzioni, agenti e personalizzazioni così sconfinata, se un utente atterra su CorpAgent e si trova davanti a tutto in un colpo solo, rischia l'effetto panico (la cosiddetta paralisi da analisi).
...
- 🧙‍♂️ Come Funziona il Wizard Guidato in CorpAgent
...
- Altre 40 funzioni definitive (tra strumenti operativi avanzati, connettori e utility di piattaforma) che completano il quadro di CorpAgent, portando la dotazione totale a un livello di ecosistema Enterprise imbattibile:
...
- Ecco l'elenco completo di tutti i connettori software e le integrazioni esterne che hai progettato per CorpAgent. Questa è la spina dorsale tecnologica che permette all'intelligenza artificiale di uscire dalla chat ed eseguire azioni concrete nel mondo reale:
